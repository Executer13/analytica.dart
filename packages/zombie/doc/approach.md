# `pkg:zombie`: Architecture, Technical Approach & Testing Standards

## 1. Executive Summary & Design Alignment

`pkg:zombie` is built as a sibling package in the `analytica.dart` monorepo workspace. It shares core design patterns, dependencies, and testing conventions with `packages/cognitive_complexity`.
...
packages/zombie/
├── bin/
│   └── zombie.dart              # CLI binary entrypoint
├── lib/
│   ├── zombie.dart              # Public programmatic API
│   └── src/
│       ├── models.dart          # DeclarationNode, ZombieFinding, OrphanTestSite, ZombieReport
...
│       ├── comment_parser.dart  # Parses // zombie:ignore directives
│       ├── formatters/
│       │   ├── json_formatter.dart
...
'''),
  ...
  check(report.zombies).single.which((it) => it
    ...
    ..classification.equals(ZombieClassification.pureZombie)
  );
});
```

### 5.2 CLI Integration Tests with `test_process`
CLI integration tests launch `bin/zombie.dart` via `TestProcess.start(Platform.resolvedExecutable, ...)` to verify:
